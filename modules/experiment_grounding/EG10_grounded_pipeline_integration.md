# EG10_grounded_pipeline_integration Grounded Pipeline Integration

## Module ID
`EG10_grounded_pipeline_integration`

## Module Name
Grounded Pipeline Integration

## Purpose
Define how experiment_context enters the existing v0.1/v0.2 pipeline and final topic package.
...
## Integration Steps
1. Read contributions from `04_contribution_argumentation/next_input.json` or validated output summaries.
2. Read reviewer risks from `05_chain_consistency_audit/next_input.json`.
3. Read optional `evidence_context` from v0.2 literature grounding.
4. Generate compact `experiment_context`.
5. Require `07_final_topic_package` to cite validation target, experiment, baseline, metric, ablation, statistical test, and artifact IDs.
6. Use `audit_claim_grounding.py` for literature evidence and `audit_validation_adequacy.py` for experiment validation planning.
